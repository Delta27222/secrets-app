...
import { useMemberships, useNotify } from "@/hooks";
...
export function OrganizationMembers({
  ...
  const { members, loading, error, userRole, fetchOrganizationMembers } = useMemberships();
  const [inviteDialogOpen, setInviteDialogOpen] =
    React.useState<boolean>(false);
  const [editDialogOpen, setEditDialogOpen] = React.useState<boolean>(false);
  const [selectedMember, setSelectedMember] =
    React.useState<OrganizationMembershipDetail | null>(null);
  ...
  React.useEffect(() => {
    if (members.length === 0) fetchOrganizationMembers(organizationId);
  }, [organizationId]);
  ...
  const handleInviteSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      notify(
        "No se pudo invitar al miembro. Por favor, intenta de nuevo más tarde.",
        "error"
      );
    } finally {
      setInviting(false);
  ...
  const handleRemoveMember = async (memberId: string) => {
    try {
      ...
      notify(
        "No se pudo eliminar al miembro. Por favor, intenta de nuevo más tarde.",
        "error"
      );
    }
  };
          ...
          <Dialog open={inviteDialogOpen} onOpenChange={setInviteDialogOpen}>
            ...
            <DialogContent>
              <DialogHeader>
                ...
                <DialogDescription>
                  Envía una invitación por correo electrónico para unirse a esta
                  organización.
                </DialogDescription>
              </DialogHeader>
      ...
      {members.length === 0 ? (
        <div className="text-center py-8">
          ...
          <p className="text-muted-foreground mb-6">
            Invita a miembros para colaborar en esta organización.
          </p>
        </div>
      ) : (
      ...
      <Dialog open={editDialogOpen} onOpenChange={setEditDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Actualiza el rol de{" "}
              {selectedMember?.user?.displayName || selectedMember?.email} en
              esta organización.
            </DialogDescription>
          </DialogHeader>
