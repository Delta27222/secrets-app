  ...
  const handleAddMember = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        ...
        description: `Se han añadido ${selectedUserIds.length} miembros al proyecto correctamente.`,
      })
    } 
    catch (err) {
        ...
        <Dialog open={addMemberDialogOpen} onOpenChange={setAddMemberDialogOpen}>
          ...
          <DialogContent className="sm:max-w-[600px]">
            <DialogHeader>
              ...
              <DialogDescription>
                Selecciona miembros de la organización y asígnales un rol en este proyecto.
              </DialogDescription>
            </DialogHeader>
                      ...
                      <CommandGroup className="max-h-[200px] overflow-y-auto">
                        {organizationMembers.map((member) => {
                          ...
                          return (
                            <CommandItem
                              key={member._id}
                              value={member.user?.displayName || member.user?.username || member.email}
                              onSelect={() => !isDisabled && toggleUserSelection(userId)}
                              className={cn(
                                "cursor-pointer",
              ...
              <DialogFooter>
                <div className="flex items-center justify-between w-full">
                  <span className="text-sm text-muted-foreground">
                    {selectedUserIds.length} {selectedUserIds.length === 1 ? 'miembro seleccionado' : 'miembros seleccionados'}
                  </span>
                  <Button 
                    type="submit" 
